...
for outer = 1:nBreg
    for inner = 1:nInner
        Sparse_Comps= 1*lambda*Dxt(x-bx,dims,use_gpu)+1*lambda*Dyt(y-by,dims,use_gpu)+1*lambda*Dzt(z-bz,dims,use_gpu)...
                     +0.2*ifftc(TE-bTE,4)+1.9*ifftc(Rep-bRep,5);
        ...
        % Sparsity transforms
        if use_gpu == 1
            dx = Dx(gpuArray(u),dims,use_gpu);
            dy = Dy(gpuArray(u),dims,use_gpu);
            dz  =Dz(gpuArray(u),dims,use_gpu);
        else
            dx = Dx((u),dims,use_gpu);
            dy = Dy((u),dims,use_gpu);
            dz  =Dz((u),dims,use_gpu);
        end
        dTE = fftc(u,4);
        dRep = fftc((u),5);
