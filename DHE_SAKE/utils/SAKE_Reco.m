function ksp = SAKE_Reco(data,varargin)
...
% default options
opts.width = [3 3 2]; % kernel width: [x y z] or scalar
opts.radial = 1; % use radial kernel [1 or 0]
opts.loraks = 0; % phase constraint (loraks)
opts.tol = 1e-4; % tolerance (fraction change in norm)
opts.maxit = 100;%1e4; % maximum no. iterations
...
opts.sparsity = 0.0; % sparsity in wavelet domain (0.1=10% zeros)
...
for iter = 1:opts.maxit
    ...
    if opts.sparsity
        ...
        ksp = gpuArray(ifft3(ksp)); % to kspace
    end
    ...
    f = max(0,1-noise_floor.^2./(1.75*W).^2); 
